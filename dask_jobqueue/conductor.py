...
conductor_cluster_parameters = """manager_rest_address: str
        ...
        Specify this value, or specify the ascd_rest_address and instance_group parameters to look up this value by the instance group name.
    ...
    ascd_rest_cacert_path: str
        ...
        Required to look up the Dask manager service REST URL by instance group name when SSL is enabled for the IBM Spectrum Conductor cluster,
        and when SSL is enabled for the Dask manager service.
    ...
    instance_group: str
        ...
        Optionally specify this value to look up the Dask manager service REST URL by instance group name.
    username: str
        Username for the IBM Spectrum Conductor cluster. 
        Required to look up the Dask manager service REST URL by instance group name, or when authentication is enabled for the instance group.
        Alternatively, set the ``EGO_USERNAME`` or ``EGO_SERVICE_CREDENTIAL`` environment variables.
    password: str
        Password for the IBM Spectrum Conductor cluster.
        Required to look up the Dask manager service REST URL by instance group name, or when authentication is enabled for the instance group.
...
def log_rest_error(resp, level=logging.ERROR):
    msg = None
    try:
        msg = _("The Dask manager returned an error. Code: %d. Message: %s") % (resp.status_code, resp.json()['message'])
    except:
...
class ConductorJob(Job):
    ...
    async def close(self):
        """
            Override Job close method to do nothing. Workers will close by themselves and be handled through EGO callback mechanism.
        """
    ...
    def fail_job(self, activity_id):
        ...
        if len(self.failed_activities) >= self._max_retries:
            self.status = "failed"
            logger.error(_("The job could not start any workers after %d tries. See worker logs for the following activity IDs for more details: %s"), len(self.failed_activities), str(self.failed_activities))
            return True
        else:
...
class ConductorCluster(JobQueueCluster):
    ...
    The ``ConductorCluster`` class interfaces with the Dask manager service running in an instance group on an IBM Spectrum Conductor cluster.
    If initialized on a host inside the IBM Spectrum Conductor cluster, the configuration from the deployed instance group will be used by default.
    ...
    >> from daskconductor.conductor import ConductorCluster
    >> cluster = ConductorCluster(instance_group_name="ig1", ascd_rest_address="http://ascd.example.com:8280/platform/rest/conductor/v1", username="MyUser", password="MyPassword")
    ...
    def __init__(
        ...
        name=None,
        asynchronous=False,
        manager_rest_address=None, #missing
        ascd_rest_address=None, #missing
        ascd_rest_cacert_path=None, #missing
        instance_group=None, #missing
        username=None, #missing
        password=None, #missing
        adapt=None, #missing
        min_workers=None, #missing
        max_workers=None, #missing
        n_workers=None,
        failed_job_limit=None, #missing
        # Scheduler keywords
        interface=None,
        host=None, #missing
        protocol=None, #missing
        dashboard_address=":8787",
        config_name=None, #missing - But it is hardcoded to config_name
        log_directory=None,
        # Job keywords
        type=None, #missing
        worker_command=None, #missing
        slots=None,
        threads=None, #missing
        memlimit=None, #missing
        resources=None, #missing
        local_directory=None,
        conda_env_path=None, #missing
        lifetime=None, #missing
        processes=None, #missing
        gpu_mode=None, #missing
        extra=None,
        env_extra=None,
        job_max_retries=None, #missing
        **kwargs
    ):
        ...
        if extra != None and not isinstance(extra, list):
            raise ValueError(_("The value '%s' specified for the 'extra' parameter is not supported. Specify a list of arguments to pass to the Dask worker. For example: ['--extra1', '--extra2 arg2'].") % extra)

        if env_extra == None:
            env_extra = dask.config.get("jobqueue.%s.env_extra" % self.config_name, None)
        if env_extra != None and not isinstance(env_extra, list):
            raise ValueError(_("The value '%s' specified for the 'env_extra' parameter is not supported. Specify a list of environment variables for the Dask worker. For example: ['ENV1=val1','ENV2=val2'].))") % env_extra)
        ...
        if failed_job_limit == None:
            self._failed_job_limit = int(dask.config.get("jobqueue.%s.failed-job-limit" % self.config_name, 5)) #Default 5
        else:
        ...
        if self.manager_rest_address == None:
            self._look_up_manager_rest_address()
            if self.manager_rest_address == None:
                raise ValueError(_("The Dask manager REST address was not specified, and could not be looked up using the ascd REST service."))
        ...
        try:
            super(ConductorCluster, self).__init__(loop=loop,
                           ...
                           n_workers=0, #We will start the n_workers after the scheduler is registered with the Dask manager
                           **kwargs)
        except ValueError:
            #In dask-jobqueue 0.7.1 there is a breaking change where dashboard_address must be passed through a new scheduler_options parameter
            #The scheduler-options parameter can also be defined in the jobqueue.yaml
            ...
            if scheduler_options == None or scheduler_options == {}:
        ...
        try:
            ...
            if n_workers != None and n_workers != 0:
    ...
    def _reinitialize_logging_with_scheduler_id(self):
        ...
        try:
            ...
            logger.info(_("ConductorCluster initialized. Scheduler: %s, Dashboard: %s"), self.scheduler.address, self.dashboard_link)
    ...
    def _submit_dask_manager_rest_request(self, method, url, data=None, headers={}, auth=None):
        ...
        resp = None
        if method == "GET":
            resp = requests.get(url, headers=headers, verify=cacert_path, cookies=self._saved_cookies, auth=auth)
        elif method == "POST":
            resp = requests.post(url, data=data, headers=headers, verify=cacert_path, cookies=self._saved_cookies, auth=auth)
        elif method == "PUT":
            resp = requests.put(url, data=data, headers=headers, verify=cacert_path, cookies=self._saved_cookies, auth=auth)
        elif method == "DELETE":
            resp = requests.delete(url, data=data, headers=headers, verify=cacert_path, cookies=self._saved_cookies, auth=auth)
    ...
    def _look_up_manager_rest_address(self):
        ...
        if self._credentials == None and (self._username == None or self._password == None):
            logger.error(_("Could not look up the Dask manager address from the ascd REST service. The username, password or credentials were not specified."))
            return None
        ...
        if self.instance_group == None:
            ...
            if self.instance_group == None:
                logger.error(_("Could not look up the Dask manager address from the ascd REST service. The instance group name was not specified."))
                return None

        #Get the cacert path from args or environment variable
        if self.ascd_rest_cacert_path == None:
        ...
        if self.ascd_rest_address != None:
            ...
            if url.port == None:
                ...
                logger.debug("Trying to look up the Dask manager address for the instance group %s from the ascd REST service at %s" % (self.instance_group, self.ascd_rest_address))
                attempted_lookup = True
                resp = self._submit_ascd_get_instances_rest_call(self.ascd_rest_address)

                if resp == None:
                    logger.error(_("Could not find the Dask manager address for the instance group %s from the ascd REST service at %s."), self.instance_group, self.ascd_rest_address)
                    return None
        ...
        if conductor_rest_address != None:
            logger.debug("Trying to look up the Dask manager address for the instance group %s from the ascd REST service at %s" % (self.instance_group, conductor_rest_address))
            attempted_lookup = True
            resp = self._submit_ascd_get_instances_rest_call(conductor_rest_address)

            if resp == None:
                logger.error(_("Could not find the Dask manager address for the instance group %s from the ascd REST service at %s."), self.instance_group, conductor_rest_address)
                return None
        ...
        if ascd_hosts != None:
            ...
            elif conductor_rest_address != None:
                base_url = urlparse(conductor_rest_address)
            else:
                logger.error(_("Could not look up the Dask manager address. The ascd REST service base URL was not specified."))
                return None

            for host in ascd_hosts.split(","):
                if host != base_url.hostname:
                    next_ascd_host_rest_address = base_url.scheme + "://" + host + ":" + str(base_url.port) + "/platform/rest"

                    logger.debug("Trying to look up the Dask manager address for the instance group %s from the ascd REST service at %s." % (self.instance_group, next_ascd_host_rest_address))
                    attempted_lookup = True
                    resp = self._submit_ascd_get_instances_rest_call(next_ascd_host_rest_address)

                    if resp == None:
                        logger.error(_("Could not find the Dask manager address for the instance group %s from the ascd REST service at %s."), self.instance_group, next_ascd_host_rest_address)
                        return None
        ...
        if attempted_lookup:
            logger.error(_("Could not find the Dask manager address for the instance group %s from any ascd host."), self.instance_group)
    ...
    def _submit_ascd_get_instances_rest_call(self, base_address):
        ...
        if self.ascd_rest_cacert_path == None:
            if base_address.startswith("https://"):
                logger.error(_("Could not look up the Dask manager address. The certificate authority (CA) certificate file for the ascd REST service was not specified."))
                return None
        else:
        ...
        if self._credentials != None:
        ...
        elif self._username != None and self._password != None:
            auth = HTTPBasicAuth(self._username, self._password)
            try:
            ...
            logger.error(_("Could not look up the Dask manager address. The username, password, or credentials were not specified."))
            return None
    ...
    def _parse_dask_manager_address_from_json(self, instance_groups):
        ...
        for ig in instance_groups:
            if ig.get('name') == self.instance_group:
                ...
                if outputs == None:
                    logger.warn(_('Could not look up the Dask manager address. The instance group %s does not have a Dask manager service.'), self.instance_group)
                    return None

                dask_manager_rest_address = outputs.get('dask_manager_rest_address')
                if dask_manager_rest_address == None:
                    logger.warn(_('Could not look up the Dask manager address. The instance group %s does not have a Dask manager service.'), self.instance_group)
                    return None

                value = dask_manager_rest_address.get('value')
                if value == None:
                    logger.warn(_("Could not look up the Dask manager address. The instance group %s does not have a running Dask manager service instance."), self.instance_group)
                    return None

                #Successfully got the address
                return value.strip('/dask/manager')

        logger.warn(_("Could not look up the Dask manager address. The instance group %s was not found."), self.instance_group)
        return None
        ...
        try:
            ...
            if len(matching_jobs) != 0:
                ...
                if len(matching_jobs) != 0:
                    ...
                    #We get to this case when the ConductorCluster has scaled down and then receives an incoming worker previously requested
    ...
    async def clean_failed_workers(self):
        ...
        if self._unable_to_start_jobs_count == self._failed_job_limit:
            logger.error(_("The ConductorCluster is not able to start any jobs after %d attempts. The cluster will close."), self._failed_job_limit)
            await self._close()
...
class ConductorPlugin(SchedulerPlugin):
    ...
    def __init__(self,
        ...
        if scheduler == None or cluster == None or manager_rest_address == None:
            raise ValueError(_("The ConductorPlugin must be initialized with scheduler, cluster and manager_rest_address."))
        ...
        if version.parse(self.distributed_version) < version.parse(self.min_distributed_version):
            raise ValueError(_("The distributed version %s is not supported. The ConductorPlugin requires distributed version %s or higher."), self.distributed_version, self.min_distributed_version)
    ...
    def remove_worker(self, scheduler=None, worker=None):
        ...
        if worker in self.cluster.workers:
            ...
            if job_id != None and job_id != "NOT_STARTED":
    ...
    async def activity_exit(self, comm, activity=None):
        ...
        if state == "ERROR" or exit_code != "0":
            logger.warn(_("The worker with the activity ID %s did not exit successfully. Exit code: %s. Reason: %s."), id, exit_code, exit_msg)
        ...
        if id in self.removed_worker_addresses:
            ...
            if len(running_workers) == 1:
                logger.warn(_("The activity with the activity ID %s has exited, but the cluster thinks it is still running. The worker's job will be cancelled."), id)
                ...
                if len(pending_jobs) > 0:
                    ...
                    #This will happen when the scheduler scales down its requested workers at the same time that EGO allocates a new worker
                    #The resulting worker will not be added to the scheduler and the activity will be stopped
                    #When the activity is stopped, we get here because the worker was never added to the scheduler
                    logger.debug("The activity with ID %s has exited, but the scheduler does not have any worker with this ID or any pending workers.", id)
            else:
    ...
    def _workers_to_reclaim(self, hostname=None, num_jobs=0):
        """
            Suggest workers to reclaim on the specified host based on workers which have the least number of tasks in progress followed by the least amount of data in memory.
        ...
        #When ConductorCluster is configured with multiple processes or threads per Job(EGO activity), each may have multiple workers associated
        #In the case of reclaim, we need to reclaim all workers for a single slot, which maps to an activity
        #Aggregate the worker stats by activity ID so that we can determine which overall activities are the least busy
        activities = []
        ...
        logger.info(_("Reclaiming %d of the %d Dask worker jobs on the host %s."), num_jobs, len(activites_sorted_by_workload), hostname)
