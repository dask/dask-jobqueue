...
class ConductorJob(Job):
    ...
    def __init__(self, *args, scheduler=None, config_name=None, **kwargs):
        super().__init__(
            *args, config_name=config_name, cores=-1, memory=-1, shebang="", **kwargs
        )
        self._max_retries = int(
            dask.config.get("jobqueue.%s.job-max-retries" % config_name, 5)
        )
    ...
    async def close(self):
        ...
        if self.status == "running" and self.job_id == "NOT_STARTED":
            ...
            logger.debug(
                "Close a job that was not started. Requested number of workers is reduced by 1."
            )
    ...
    def fail_job(self, activity_id):
        ...
        if len(self.failed_activities) >= self._max_retries:
            ...
            logger.error(
                _(
                    "The job could not start any workers after %d tries. "
                    "See worker logs for the following activity IDs for more details: "
                    "%s"
                ),
                len(self.failed_activities),
                str(self.failed_activities),
            )
            return True
        else:
    ...
    def _send_dask_manager_workers_rest_request(self, num_workers=0):
        ...
        manager_rest_address = dask.config.get(
            "jobqueue.conductor_internal.manager_rest_address"
        )
        ...
        url = (
            manager_rest_address
            + "/dask/manager/schedulers/"
            + scheduler_id
            + "/workers"
        )
        ...
        resp = requests.put(
            url, data=str(data), headers=headers, cookies=cookies, verify=cacert
        )
...
class ConductorCluster(JobQueueCluster):
    ...
    """.format(
        conductor_cluster=conductor_cluster_parameters,
        job=job_parameters,
    ...
    def __init__(
        ...
        **kwargs
        ...
        if n_workers is None:
            n_workers = dask.config.get(
                "jobqueue.%s.start-workers" % self.config_name, None
            )
            if n_workers is None:
                n_workers = 0
        ...
        if worker_command is None:
            worker_command = dask.config.get(
                "jobqueue.%s.worker_command" % self.config_name, None
            )
        ...
        if resources is None:
            resources = dask.config.get(
                "jobqueue.%s.resources" % self.config_name, None
            )

        if processes is None:
            processes = dask.config.get(
                "jobqueue.%s.processes" % self.config_name, None
            )
        ...
        if interface is None:
            interface = dask.config.get(
                "jobqueue.%s.interface" % self.config_name, None
            )

        if conda_env_path is None:
            conda_env_path = dask.config.get(
                "jobqueue.%s.conda-env-path" % self.config_name, None
            )

        if local_directory is None:
            local_directory = dask.config.get(
                "jobqueue.%s.local-directory" % self.config_name, None
            )
        ...
        if extra is not None and not isinstance(extra, list):
            raise ValueError(
                _(
                    "The value '%s' specified for the 'extra' parameter is not supported. "
                    "Specify a list of arguments to pass to the Dask worker. "
                    " For example: ['--extra1', '--extra2 arg2']."
                )
                % extra
            )
        ...
        if env_extra is None:
            env_extra = dask.config.get(
                "jobqueue.%s.env_extra" % self.config_name, None
            )
        if env_extra is not None and not isinstance(env_extra, list):
            raise ValueError(
                _(
                    "The value '%s' specified for the 'env_extra' parameter is not supported. "
                    "Specify a list of environment variables for the Dask worker. "
                    "For example: ['ENV1=val1','ENV2=val2'].))"
                )
                % env_extra
            )
        ...
        if log_directory is None:
            log_directory = dask.config.get(
                "jobqueue.%s.log-directory" % self.config_name, None
            )

        # Get other cluster parameters from conf file if not explicitly specified
        if failed_job_limit is None:
            self._failed_job_limit = int(
                dask.config.get("jobqueue.%s.failed-job-limit" % self.config_name, 5)
            )
        else:
        ...
        if self.manager_rest_address is None:
            ...
            if self.manager_rest_address is None:
                raise ValueError(
                    _(
                        "The Dask manager REST address was not specified, "
        ...
        try:
            super(ConductorCluster, self).__init__(
                loop=loop,
                security=security,
            ...
            scheduler_options = dask.config.get(
                "jobqueue.%s.scheduler-options" % self.config_name, None
            )
        ...
        try:
            ...
            dask.config.set(
                {
                    "jobqueue.conductor_internal.manager_rest_address": self.manager_rest_address,
            ...
            if job_max_retries is not None:
                dask.config.set(
                    {"jobqueue.%s.job-max-retries" % config_name: job_max_retries}
                )
            ...
            if min_workers is None:
                min_workers = dask.config.get(
                    "jobqueue.%s.min-workers" % self.config_name, None
                )
                if min_workers is not None:
                    min_workers = int(min_workers)

            if max_workers is None:
                max_workers = dask.config.get(
                    "jobqueue.%s.max-workers" % self.config_name, None
                )
    ...
    def _reinitialize_logging_with_scheduler_id(self):
        ...
        try:
            ...
            new_filename = (
                self.scheduler.id + "__" + hostname + "__" + username + ".log"
            )
            new_log_dir = None
            if self.log_directory is not None:
                ...
                dask.config.set(
                    {
                        "jobqueue.%s.log-directory"
                        % (self.config_name): self.log_directory
                    }
                )
            else:
            ...
            logger.info(
                _("ConductorCluster initialized. Scheduler: %s, Dashboard: %s"),
    ...
    def _initial_logon_dask_manager(self):
        ...
        if self._username is not None and self._password is not None:
            ...
            headers.update(
                {"Authorization": "PlatformToken token=" + self._credentials}
            )
            self.do_auth = True

        if self.do_auth:
            auth_url = self.manager_rest_address + "/dask/manager/auth/logon"
            resp = self._submit_dask_manager_rest_request(
                "GET", auth_url, headers=headers, auth=auth
            )
            if resp.status_code != requests.codes.ok:
                ...
                dask.config.set(
                    {
                        "jobqueue.conductor_internal.saved_cookies": self._saved_cookies,
    ...
    def _renew_token(self):
        ...
        if self.do_auth:
            ...
            if resp.status_code != requests.codes.ok:
                ...
                self._csrftoken = resp.json()["csrftoken"]
                self._saved_cookies["platform.rest.token"] = resp.cookies[
                    "platform.rest.token"
                ]
    ...
    def _unregister_scheduler_from_manager(self, ignore_rest_errors=True):
        ...
        """
        url = (
            self.manager_rest_address + "/dask/manager/schedulers/" + self.scheduler.id
        )
    ...
    def _submit_dask_manager_rest_request(
        self, method, url, data=None, headers={}, auth=None
    ):
        """
    ...
    def _look_up_manager_rest_address(self):
        ...
        if self._credentials is None and (
            ...
            logger.error(
                _(
                    "Could not look up the Dask manager address from the ascd REST service. "
        ...
        if self.instance_group is None:
            self.instance_group = dask.config.get(
                "jobqueue.%s.instance-group" % self.config_name, None
            )
            if self.instance_group is None:
                logger.error(
                    _(
        ...
        if self.ascd_rest_address is not None:
            ...
            if url.port is None:
                logger.error(
                    _("The ascd REST address %s does not contain a port number."),
                    self.ascd_rest_address,
                )
            else:
                if url.path == "":
                ...
                logger.debug(
                    "Trying to look up the Dask manager address for the instance group %s from the "
                    "ascd REST service at %s"
                    % (self.instance_group, self.ascd_rest_address)
                )
                ...
                if resp is None:
                    logger.error(
                        _(
                            "Could n find the Dask manager address for the instance group %s from the "
                            "ascd REST service at %s."
                        ),
                        self.instance_group,
                        self.ascd_rest_address,
                ...
                if resp is not False:
                    if resp.status_code == requests.codes.ok:
                        self.manager_rest_address = (
                            self._parse_dask_manager_address_from_json(resp.json())
                        )
                        return
                    else:
                        log_rest_error(resp)
        ...
        if conductor_rest_address is not None:
            logger.debug(
                "Trying to look up the Dask manager address for the instance group %s from the "
                "ascd REST service at %s"
                % (self.instance_group, conductor_rest_address)
            )
            attempted_lookup = True
            ...
            if resp is None:
                logger.error(
                    _(
                        "Could not find the Dask manager address for the instance group %s from the "
                        "ascd REST service at %s."
                        ""
                    ),
                    self.instance_group,
                    conductor_rest_address,
                )
                return None
        ...
        if ascd_hosts is not None:
            ...
            if self.ascd_rest_address is not None:
                ...
                logger.error(
                    _(
                        "Could not look up the Dask manager address. "
            ...
            for host in ascd_hosts.split(","):
                if host != base_url.hostname:
                    next_ascd_host_rest_address = ("%s://%s:%d/platform/rest") % (
                        base_url.scheme,
                        host,
                        base_url.port,
                    )
                    ...
                    logger.debug(
                        "Trying to look up the Dask manager address for the instance group %s from the "
                        "ascd REST service at %s."
                        % (self.instance_group, next_ascd_host_rest_address)
                    )
                    attempted_lookup = True
                    resp = self._submit_ascd_get_instances_rest_call(
                        next_ascd_host_rest_address
                    )
        ...
        if attempted_lookup:
            logger.error(
                _(
                    "Could not find the Dask manager address for the instance group %s from any ascd host."
                ),
                self.instance_group,
            )
    ...
    def _submit_ascd_get_instances_rest_call(self, base_address):
        ...
        if self.ascd_rest_cacert_path is None:
            if base_address.startswith("https://"):
                logger.error(
                    _(
                        "Could not look up the Dask manager address. The certificate authority "
        ...
        if self._credentials is not None:
            ...
            try:
                ...
                logger.warn(
                    _("Could not connect to the ascd REST service at %s. %s"),
                    auth_url,
                    message,
                )
                return False
            ...
            resp = requests.get(
                url, headers=headers, cookies=auth_resp.cookies, verify=verify
            )
            if resp.status_code != requests.codes.ok:
                log_rest_error(resp)
                return None
    ...
    def _parse_dask_manager_address_from_json(self, instance_groups):
            ...
            if ig.get("name") == self.instance_group:
                ...
                if outputs is None:
                    logger.warn(
                        _(
                            "Could not look up the Dask manager address. "
                            "The instance group %s does not have a Dask manager service."
                        ),
                        self.instance_group,
                    )
                    return None
        ...
        logger.warn(
            _(
                "Could not look up the Dask manager address. The instance group %s was not found."
            ),
            self.instance_group,
        )
        return None
    ...
    def scale(self, n=0, jobs=None, slots=None):
        ...
        if len(self.worker_spec) > n:
            not_yet_launched = set(self.worker_spec) - set(
    ...
    def update_launched_job(self, worker_address=None, worker_name=None):
        ...
        try:
            ...
            matching_jobs = (
                self.get_workers_with_job_id()
            )  # Get workers with NOT_STARTED job id
            ...
            if len(matching_jobs) != 0:
                ...
                matching_jobs = set(
                    filter(lambda job: job.job_id == activity_id, self.workers.values())
                )
                if len(matching_jobs) != 0:
                    ...
                    if job.status == "running":
                        ...
                        logger.warn(
                            _("The job with activity ID %s is not running.")
                            % activity_id
                        )
                else:
                    ...
                    logger.info(
                        _(
                            "There are no jobs waiting to be launched. Closing the Dask worker %s."
                        ),
                        worker_name,
                    )
                    self.loop.add_callback(
                        self.retire_workers, workers=[worker_address]
                    )
    ...
    async def retire_workers(self, workers=None):
        ...
        closed_workers = await self.scheduler.retire_workers(
            workers=workers, retire=True, close_workers=True
        )
    ...
    async def clean_failed_workers(self):
        ...
        if self._unable_to_start_jobs_count == self._failed_job_limit:
            logger.error(
                _(
                    "The ConductorCluster is not able to start any jobs after %d attempts. "
                    "The cluster will close."
                ),
                self._failed_job_limit,
            )
            await self._close()
...
class ConductorPlugin(SchedulerPlugin):
    ...
    def __init__(self, scheduler=None, cluster=None, manager_rest_address=None):

        if scheduler is None or cluster is None or manager_rest_address is None:
            raise ValueError(
                _(
                    "The ConductorPlugin must be initialized with scheduler, "
        ...
        self.removed_worker_addresses = (
            []
        )  # String of worker job/activity IDs that have been removed by the scheduler

        self.distributed_version = self._get_distributed_version()
        if version.parse(self.distributed_version) < version.parse(
            self.min_distributed_version
        ):
            raise ValueError(
                _(
                    "The distributed version %s is not supported. "
        ...
        self.scheduler.handlers.update(
            {
                "reclaim-workers": self.reclaim_workers,
    ...
    def add_worker(self, scheduler=None, worker=None, **kwargs):
        ...
        """
        logger.debug(
            "Add a Dask worker %s with name %s.", worker, scheduler.workers[worker].name
        )
    ...
    def remove_worker(self, scheduler=None, worker=None):
        ...
        if worker in self.cluster.workers:
            ...
            if job_id is not None and job_id != "NOT_STARTED":
                ...
                logger.warn(
                    _("The worker with address %s does not have a job ID."), worker
                )
    ...
    async def activity_exit(self, comm, activity=None, **kwargs):
        ...
        if id in self.removed_worker_addresses:
            ...
            if len(running_workers) == 1:
                logger.warn(
                    _(
                        "The activity with the activity ID %s has exited, but the cluster thinks it is "
                        "still running. The worker's job will be cancelled."
                    ),
                    id,
                )
                ...
                if len(pending_jobs) > 0:
                    logger.debug(
                        "Cancel one of the jobs that has not started as a result of failed worker activity."
                    )
                    ...
                    logger.debug(
                        "The activity with ID %s has exited, but the scheduler does not have any worker "
                        "with this ID or any pending workers.",
                        id,
                    )
            else:
    ...
    def _reclaim_workers(self, reclaim=None):
        ...
        for hostname in reclaim_map:
            ...
            reclaim_candidate_workers.extend(
                self._workers_to_reclaim(hostname, num_workers)
            )
    ...
    def _workers_to_reclaim(self, hostname=None, num_jobs=0):
        ...
        workers_on_host = list(
            filter(
                lambda ws: getattr(ws, "name").startswith("Worker__" + hostname),
        ...
        workers_sorted_by_activity = sorted(
            workers_on_host,
            key=lambda ws: get_activity_id_from_worker_name(getattr(ws, "name")),
        )
        ...
        activites_sorted_by_workload = sorted(
            activities, key=lambda a: (a["has_what"], a["processing"]), reverse=False
        )
        logger.info(
            _("Reclaiming %d of the %d Dask worker jobs on the host %s."),
            num_jobs,
            len(activites_sorted_by_workload),
            hostname,
        )
        ...
        for a in activites_sorted_by_workload[0:num_jobs]:
            logger.debug(
                "Reclaiming the EGO activity with ID %s, which has %d workers."
                % (a["id"], len(a["addresses"]))
            )
            addresses += a["addresses"]

        logger.info(
            _("As part of the reclaim, %d workers will be closed."), len(addresses)
        )
