...
function jobqueue_install {
    cd ./ci/htcondor
    docker-compose exec -T submit /bin/bash -c "conda activate dask-jobqueue; cd /dask-jobqueue; pip3 install -e .;chown -R submituser ."
    cd -
}

function jobqueue_script {
    cd ./ci/htcondor
    docker-compose exec -T --user submituser submit /bin/bash -c "cd; conda activate dask-jobqueue; pytest /dask-jobqueue/dask_jobqueue --log-cli-level DEBUG --capture=tee-sys --verbose -E htcondor "
    cd -
}
